...
fn get_page_size() -> u64 {
    ...
    let ret = unsafe {
        raw::sysctlbyname(
            name.as_ptr(),
            &mut pagesize as *mut _ as *mut c_void,
            &mut len,
            std::ptr::null(),
            0,
        )
...
pub fn get_network_counters() -> Result<NetworkCounters, FfiError> {
    let mut mib: [i32; 6] = [raw::CTL_NET, raw::PF_ROUTE, 0, 0, raw::NET_RT_IFLIST2, 0];
    ...
    while off + 4 <= len {
        ...
        if mtype == raw::RTM_IFINFO2 && off + hdr_size <= len {
            ...
            let hdr =
                unsafe { std::ptr::read_unaligned(buf.as_ptr().add(off) as *const raw::IfMsghdr2) };
...
pub fn get_disk_counters() -> Result<DiskCounters, FfiError> {
    ...
    loop {
        ...
        let props =
            unsafe { raw::IORegistryEntryCreateCFProperty(drive, stats_key, std::ptr::null(), 0) };
        if !props.is_null() {
