...
pub fn get_process_cwd(pid: i32) -> Result<String, FfiError> {
    ...
    let end = info
        ...
        .iter()
        .position(|&b| b == 0)
        .unwrap_or(info.pvi_cdir.vip_path.len());
...
fn resolve_socket_detail(pid: i32, fd: i32) -> Result<NetworkConnection, FfiError> {
    ...
    let in_info: raw::InSockInfo =
        unsafe { std::ptr::read(info.psi.soi_proto.as_ptr() as *const raw::InSockInfo) };
    ...
    let state = if sock_type == 1 {
        ...
        let tcp_info: raw::TcpSockInfo =
            unsafe { std::ptr::read(info.psi.soi_proto.as_ptr() as *const raw::TcpSockInfo) };
