...
fn network_and_disk_counters_are_plausible() {
    ...
    assert!(
        disk.read_bytes > 0,
        "expected non-zero disk reads since boot"
    );
}
...
fn get_process_info_returns_ppid_and_threads() {
    ...
    assert!(
        info.state == prexp_ffi::ProcessState::Running
            || info.state == prexp_ffi::ProcessState::Sleeping,
...
fn get_process_detail_for_self() {
    let my_pid = std::process::id() as i32;
    let detail =
        prexp_ffi::get_process_detail(my_pid, "test_parent").expect("failed to get process detail");
...
"#,
    ...
    assert!(
        total_syscalls > 50_000,
        "total syscalls should be >50K, got {}",
        total_syscalls
    );
    assert!(
        total_syscalls < 100_000_000,
        "total syscalls should be <100M, got {}",
        total_syscalls
    );
