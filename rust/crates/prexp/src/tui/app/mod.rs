...
pub const SIGNALS: &[Signal] = &[
    ...
    Signal {
        number: 1,
        name: "SIGHUP",
        description: "Hangup / reload config",
    },
    Signal {
        number: 2,
        name: "SIGINT",
        description: "Interrupt (like Ctrl+C)",
    },
    Signal {
        number: 19,
        name: "SIGSTOP",
...
impl App {
        ...
        for snap in snapshots {
            ...
            self.prev_counters.insert(
                ...
                ProcessCounters {
                    ...
                    syscalls: snap.activity.syscalls_mach as i64
                        + snap.activity.syscalls_unix as i64,
        ...
        if !self.search_text.is_empty() && self.main_view == MainView::Processes {
            ...
            self.filtered_indices = visible
                ...
                .filter(|&i| {
                    ...
                    s.name.to_lowercase().contains(&query) || s.pid.to_string().contains(&query)
                })
                .collect();
        } else {
    ...
    fn rebuild_file_list(&mut self) {
        ...
        for snap in &self.snapshots {
            ...
            for res in &snap.resources {
                if let Some(path) = &res.path {
                    let entry = path_map
                        .entry(path.clone())
                        .or_insert_with(|| (res.kind.clone(), Vec::new()));
        ...
        let mut entries: Vec<FileEntry> = path_map
            .into_iter()
            .map(|(path, (kind, openers))| FileEntry {
                path,
                kind,
                openers,
            })
    ...
    pub fn kill_select(&mut self) {
        if let Some(KillState::Picking { selected }) = self.kill_state {
            if selected < SIGNALS.len() {
                ...
                // Custom
                self.kill_state = Some(KillState::CustomInput {
                    input: String::new(),
                });
            }
        }
    ...
    pub fn kill_confirm(&mut self) {
        ...
        ) = (&self.kill_state, self.kill_target_pid)
        {
            ...
            match result {
                Ok(()) => {
                    self.status_message =
                        Some(format!("Sent {} to {} (pid {})", signal_name, name, pid));
                }
                Err(e) => {
