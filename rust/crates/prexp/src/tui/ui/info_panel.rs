...
fn overview_lines(
    ...
    vec![
        ...
        kv(
            "Parent",
            &format!("{} (pid {})", detail.parent_name, detail.ppid),
        ),
        kv("Path", &detail.path),
        kv("CWD", &detail.cwd),
        kv("User", &format!("{} (uid {})", detail.user, detail.uid)),
        kv("State", detail.state.label()),
...
fn resources_lines(
    ...
    let mut lines = vec![
        ...
        kv2(
            "Threads",
            &detail.thread_count.to_string(),
            "Files",
            &detail.fd_files.to_string(),
        ),
        kv2(
            "Virtual",
            &app::format_memory(detail.virtual_size),
            "Sockets",
            &detail.fd_sockets.to_string(),
        ),
        kv2(
            "RSS",
            &app::format_memory(detail.memory_rss),
            "Pipes",
            &detail.fd_pipes.to_string(),
        ),
        kv2(
            "PMEM",
            &app::format_memory(detail.memory_phys),
            "Other",
            &detail.fd_other.to_string(),
        ),
    ...
    if let Some(history) = app.process_history.get(&detail.pid) {
        ...
        let mem_pcts: Vec<f64> = if let Some(&max) = history.memory.iter().max() {
            if max > 0 {
                history
                    .memory
                    .iter()
                    .map(|&m| (m as f64 / max as f64) * 100.0)
                    .collect()
            } else {
        ...
        if let Some(&peak) = history.memory.iter().max() {
            lines.push(Line::from(Span::styled(
                format!(
                    "  now: {}  peak: {}",
                    app::format_memory(mem_now),
                    app::format_memory(peak)
                ),
                Style::default().fg(t.muted),
            )));
        }
        ...
        if cc.is_enabled(Chart::DiskIo)
            && (!history.disk_read_rate.is_empty() || !history.disk_write_rate.is_empty())
        {
            add_dual_chart(
                &mut lines,
                "Disk I/O (rate)",
...
fn add_dual_chart(
    lines: &mut Vec<Line<'static>>,
    title: &str,
    series1: (&str, &VecDeque<f64>),
    series2: (&str, &VecDeque<f64>),
    fmt_val: impl Fn(f64) -> String,
    t: &super::super::theme::Theme,
) {
    let (label1, data1) = series1;
    let (label2, data2) = series2;
    lines.push(Line::from(""));
    lines.push(section_header(title, t));
    ...
    lines.push(Line::from(vec![
        ...
        sparkline_span(&d1, t),
        Span::styled(
            format!("  now: {}  peak: {}", fmt_val(now1), fmt_val(peak1)),
            Style::default().fg(t.muted),
        ),
    ]));
    ...
    lines.push(Line::from(vec![
        ...
        sparkline_span(&d2, t),
        Span::styled(
            format!("  now: {}  peak: {}", fmt_val(now2), fmt_val(peak2)),
            Style::default().fg(t.muted),
        ),
    ]));
}
...
fn network_lines(
    ...
    let active = detail
        .network
        .iter()
        .filter(|c| matches!(c.state.as_deref(), Some("ESTABLISHED") | Some("LISTEN")))
        .count();
    let total = detail.network.len();
    let header = format!("NETWORK CONNECTIONS ({} active, {} total)", active, total);

    let mut lines = vec![Line::from(""), section_header(&header, t), Line::from("")];
    ...
    if detail.network.is_empty() {
        ...
        for conn in &detail.network {
            ...
            let line = if remote.is_empty() {
                ...
                format!(
                    "  {:<5} {:<25} → {:<25} {}",
                    conn.proto, conn.local_addr, remote, state_str
                )
            };
