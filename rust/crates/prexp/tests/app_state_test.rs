...
use prexp_core::models::{
    DiskIo, OpenResource, ProcessActivity, ProcessMemory, ProcessSnapshot, ProcessState,
    ResourceKind,
};

use prexp_app::tui::app::{
    App, Chart, ChartConfig, Column, FileSortField, InputMode, MainView, ProcessSortField,
    SortDirection,
};
...
fn tree_view_groups_children_under_parent() {
    ...
    assert_eq!(
        app.snapshots[app.tree_entries[0].snapshot_index].name,
        "nginx"
    );
    assert_eq!(app.tree_entries[0].depth, 0);

    assert_eq!(
        app.snapshots[app.tree_entries[1].snapshot_index].name,
        "node"
    );
...
fn tree_entries_have_correct_prefixes() {
    ...
    assert!(
        app.tree_entries[1].prefix.contains("└── ") || app.tree_entries[1].prefix.contains("├── ")
    );
}
...
fn file_anchor_survives_refresh() {
    ...
    app.refresh(&source);
    assert_eq!(app.selected_file_entry().unwrap().path, anchored_path);
}
...
fn file_sort_by_filename() {
    ...
    for window in filenames.windows(2) {
        assert!(
            window[0] <= window[1],
            "should be sorted by filename asc: {:?}",
            filenames
        );
    }
}
...
fn disabled_chart_skips_new_data() {
    ...
    for history in app.process_history.values() {
        assert!(
            history.threads.is_empty(),
            "disabled chart should not collect data"
        );
    }
}
...
fn enabled_chart_collects_history() {
    ...
    let fd_idx = Chart::ALL
        .iter()
        .position(|&c| c == Chart::FdCount)
        .unwrap();
    ...
    for history in app.process_history.values() {
        assert!(
            !history.fd_count.is_empty(),
            "enabled chart should collect data"
        );
    }
}
