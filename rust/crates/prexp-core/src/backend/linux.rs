...
impl ProcessSource for LinuxProcessSource {
    ...
    fn system_battery(&self) -> Result<crate::system::BatteryInfo, PrexpError> {
        ...
        let percent =
            num("capacity").ok_or_else(|| PrexpError::Backend("no battery capacity".into()))?;
    ...
    fn process_detail(&self, pid: i32, parent_name: &str) -> Result<ProcessDetail, PrexpError> {
        let proc = Process::new(pid).map_err(|e| proc_err_to_prexp(e, pid))?;
        let stat = proc.stat().map_err(|e| proc_err_to_prexp(e, pid))?;

        let status = proc.status().ok();

        let uid = status.as_ref().map(|s| s.ruid as u32).unwrap_or(0);
        ...
        let context_switches = status
            .as_ref()
            .map(|s| {
                (s.voluntary_ctxt_switches.unwrap_or(0) + s.nonvoluntary_ctxt_switches.unwrap_or(0))
                    as i32
            })
            .unwrap_or(0);

        let (disk_bytes_read, disk_bytes_written) = proc
            .io()
            .map(|io| (io.read_bytes, io.write_bytes))
            .unwrap_or((0, 0));

        // Environment.
        let environment: Vec<(String, String)> = proc
            .environ()
            .ok()
            .map(|env| {
                env.into_iter()
...
fn snapshot_process(proc: &Process) -> Result<ProcessSnapshot, PrexpError> {
    ...
    let (disk_read, disk_written) = proc
        .io()
        .map(|io| (io.read_bytes, io.write_bytes))
        .unwrap_or((0, 0));
